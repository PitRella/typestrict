...
class TestMustAnnotateConfig:
    """Unit tests for MustAnnotateConfig behaviour."""
    ...
    def test_custom_ignore(self) -> None:
        config = MustAnnotateConfig(ignore=["TF001"])
    ...
    def test_strict_removes_tf005_from_ignore(self) -> None:
        config = MustAnnotateConfig.from_dict({"strict": True, "ignore": ["TF005"]})
        assert "TF005" not in config.ignore

    def test_strict_false_keeps_tf005_in_ignore(self) -> None:
        config = MustAnnotateConfig.from_dict({"strict": False, "ignore": ["TF005"]})
        assert "TF005" in config.ignore


class TestMatchesPattern:
    """Unit tests for MustAnnotateConfig._matches_pattern."""

    def test_directory_pattern(self) -> None:
        assert MustAnnotateConfig._matches_pattern("app/migrations/0001.py", "migrations/")

    def test_filename_pattern(self) -> None:
        assert MustAnnotateConfig._matches_pattern("/project/tests/conftest.py", "conftest.py")

    def test_no_match(self) -> None:
        assert not MustAnnotateConfig._matches_pattern("app/models.py", "migrations/")
...
class TestLoadConfig:
    ...
    def test_load_from_pyproject(self, tmp_path: Path) -> None:
        ...
        from must_annotate.config import load_config
    ...
    def test_defaults_when_no_pyproject(self, tmp_path: Path) -> None:
        ...
        assert isinstance(config, MustAnnotateConfig)
