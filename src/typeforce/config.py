...
class _TomlSection(TypedDict, total=False):
    """Shape of the raw ``[tool.typeforce]`` TOML table."""
...
class TypeforceConfig:
    ...
    exclude: list[str] = field(default_factory=lambda: list(TypeforceConfig.DEFAULT_EXCLUDE))
    ignore: list[str] = field(default_factory=lambda: list(TypeforceConfig.DEFAULT_IGNORE))
    per_file_ignores: dict[str, list[str]] = field(default_factory=dict)
    strict: bool = False

    @classmethod
    def from_dict(cls, section: _TomlSection) -> TypeforceConfig:
    ...
    @classmethod
    def from_pyproject(cls, search_path: Path | None = None) -> TypeforceConfig:
        """Load config from the nearest ``pyproject.toml``.

        Searches from *search_path* (defaults to cwd) upward until a
        ``pyproject.toml`` with a ``[tool.typeforce]`` section is found.
        ...
        for directory in [start, *start.parents]:
            ...
            if candidate.exists():
                try:
                    ...
                    section: _TomlSection = data.get("tool", {}).get("typeforce", {})
...
def load_config(search_path: Path | None = None) -> TypeforceConfig:
    """Convenience wrapper around ``TypeforceConfig.from_pyproject``."""
    return TypeforceConfig.from_pyproject(search_path)
